...
namespace KBEngine
{
    ...
    public class MessageReader
    {
		...
		public void process(byte[] datas, MessageLength length, Dictionary<uint, MessageHandler> flowHandler) {
			...
			if (responseFlag != 0) {
				KBEngineApp.app.queueInLoop( delegate() {
					if(responseFlag == - 10008){
						var tips = WindowMng.windowMng.PushView("UI/systemtips");
						var alpha = Util.FindChildRecursive(tips.transform, "text").GetComponent<TweenAlpha>();
						alpha.ResetToBeginning();
						alpha.enabled = true;
						alpha.PlayForward();
						var str = Util.FindChildRecursive(tips.transform, "text").GetComponent<UILabel>();
						str.text = "你不在队伍";
					}
				});
