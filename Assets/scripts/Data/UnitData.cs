 ...
 */
	public class UnitData
	{
		...
		public Treasure GetRandomDrop ()
		{
			...
			var rd = Random.Range (0, total);
			int lastWeight = 0;
			for (int i = 0; i < TreasureData.Count; i++) {
				if(rd >= lastWeight && rd <  TreasureData[i].Weight+lastWeight) {
					return TreasureData[i];
				}
				lastWeight += TreasureData[i].Weight;
			}
			*/
			return null;
		}
